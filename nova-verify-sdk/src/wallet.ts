...
export class WalletService {
  ...
  public async connect(walletType: 'freighter' | 'rabby' | 'metamask' = 'freighter'): Promise<WalletConnection> {
    try {
      ...
      if (typeof localStorage !== 'undefined') {
        localStorage.setItem('nova-verify-wallet-connection', JSON.stringify({
          ...this.connection,
          timestamp: Date.now()
        }));
      }
  ...
  public async disconnect(): Promise<void> {
    try {
      if (this.currentWallet) {
        ...
        if (typeof localStorage !== 'undefined') {
          localStorage.removeItem('nova-verify-wallet-connection');
        }
      }
    } catch (error) {
      throw ErrorHandler.wrap(async () => { throw error; });
  ...
  public async autoConnect(): Promise<WalletConnection | null> {
    try {
      ...
      const storedConnection = localStorage.getItem('nova-verify-wallet-connection');
      if (!storedConnection) {
        return null;
      }
