 ...
 */
export class Config {
  private options: Required<Nova VerifyOptions>;
   ...
   */
  private mergeWithDefaults(options: Nova VerifyOptions): Required<Nova VerifyOptions> {
    return {
      apiEndpoint: options.apiEndpoint || 'https://api.nova-verify.com',
   ...
   * Get current options
   */
  public getOptions(): Nova VerifyOptions {
    return { ...this.options };
  }
   ...
   */
  public update(options: Partial<Nova VerifyOptions>): void {
   ...
   */
  public getRetryConfig(): Required<Nova VerifyOptions>['retry'] {
   ...
   */
  public getWalletConfig(): Required<Nova VerifyOptions>['wallet'] {
  ...
  constructor(config: Config) {
    ...
    if (config.isLoggingEnabled()) {
      this.client.interceptors.request.use(
        (request) => {
          console.log(`[Nova Verify SDK] ${request.method?.toUpperCase()} ${request.url}`);
          return request;
        },
        (error) => {
          console.error('[Nova Verify SDK] Request error:', error);
      ...
      this.client.interceptors.response.use(
        (response) => {
          console.log(`[Nova Verify SDK] Response: ${response.status} ${response.config.url}`);
          return response;
        },
        (error) => {
          console.error('[Nova Verify SDK] Response error:', error.response?.status, error.message);
          return Promise.reject(error);
        }
      );
    }
      ...
      try {
        ...
        if (attempt < maxAttempts) {
          ...
          if (this.config.isLoggingEnabled()) {
            console.log(`[Nova Verify SDK] Retry attempt ${attempt}/${maxAttempts} in ${retryDelay}ms`);
          }
        }
      }
    }
