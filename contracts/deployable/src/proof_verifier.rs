...
/// Compute SHA-256 hash from event data
fn compute_proof_hash(env: &Env, event_data: &Bytes) -> BytesN<32> {
    env.crypto().sha256(event_data)
}
...
impl ProofVerifier {
    ...
    pub fn issue_proof(env: Env, issuer: Address, request: ProofRequest) -> u64 {
        ...
        let hash = compute_proof_hash(&env, &request.event_data);
        ...
        env.events().publish(
            (symbol_short!("prf_iss"), proof_id, issuer),
    ...
    pub fn verify_proof(env: Env, verifier: Address, proof_id: u64) -> bool {
        ...
        let computed_hash = compute_proof_hash(&env, &proof.event_data);
        ...
        if !proof.verified {
            ...
            env.events().publish(
                (symbol_short!("prf_vrfy"), proof_id, verifier),
    ...
    pub fn revoke_proof(env: Env, revoker: Address, proof_id: u64, reason: String) {
        ...
        env.events().publish(
            (symbol_short!("prf_revk"), proof_id, revoker),
    ...
    pub fn get_revoked_proofs(env: Env) -> Vec<Proof> {
        ...
        for proof_id in revoked_ids.iter() {
            let id: u64 = proof_id;
            if let Some(proof) = env.storage().instance().get::<DataKey, Proof>(&DataKey::Proof(id)) {
                proofs.push_back(proof);
            }
        }
